...
Game.prototype.aiPlayCards = function(player) {
  ...
  if (toolIdx >= 0) {
    ...
    // 顺手牵羊需要距离1内
    const inRange = (p) => card.type === 'guohe' || this.calcDistance(player, p) <= 1;
    const targets = this.players.filter(p => p.alive && p.id !== player.id && p.hero.id !== 'jiaxu' && inRange(p) && this.buildGuoheChoices(p).length > 0 && (!this.isDouDizhu || this.getEnemies(player).some(e => e.id === p.id)));
    if (targets.length > 0) {
      const target = targets.reduce((a, b) => a.hp >= b.hp ? a : b);
      this.discardCard(player, card);
  ...
  if (juedouIdx >= 0) {
    ...
    // 诸葛亮空城：无手牌时不能成为【决斗】目标
    const targets = this.players.filter(p => p.alive && p.id !== player.id && p.hero.id !== 'jiaxu' && !(p.hero.id === 'zhugeliang' && p.hand.length === 0) && (!this.isDouDizhu || this.getEnemies(player).some(e => e.id === p.id)));
    if (targets.length > 0) {
  ...
  if (canSha && !kejiHold) {
    ...
    // 关羽AI：如果没有普通杀，找红牌当杀；赵云AI：闪可当杀
    if (shaIdx < 0 && player.hero.id === 'guanyu') {
      shaIdx = player.hand.findIndex(c => c.type !== 'sha' && isRedSuit(c.suit));
    }
    if (shaIdx < 0 && player.hero.id === 'zhaoyun') {
      shaIdx = player.hand.findIndex(c => c.type === 'shan');
    }
    if (shaIdx >= 0) {
      const card = player.hand[shaIdx];
      const shaRange = this.getShaRange(player);
      // 诸葛亮空城：无手牌时不能成为【杀】目标
      const targets = this.players.filter(p => p.alive && p.id !== player.id && !(p.hero.id === 'zhugeliang' && p.hand.length === 0) && this.calcDistance(player, p, true) <= shaRange && (!this.isDouDizhu || this.getEnemies(player).some(e => e.id === p.id)));
      if (targets.length > 0) {
        const target = targets.reduce((a, b) => a.hp <= b.hp ? a : b);
        this.discardCard(player, card);
        ...
        const isSkillSha = card.type !== 'sha' && (player.hero.id === 'guanyu' || player.hero.id === 'zhaoyun');
        const skillShaName = player.hero.id === 'zhaoyun' ? '【龙胆】' : '【武圣】';
        if (isSkillSha) this.addLog(`${player.hero.name}发动${skillShaName}将${card.suit}【${card.name}】当【杀】使用`, 'skill');
  ...
  // 曹仁据守：体力较低时摸3张牌并结束出牌阶段
  if (player.hero.id === 'caoren' && !this.jushouUsedThisTurn && player.hp <= 2) {
    this.doJushou(player);
    this.addLog(`${player.hero.name}跳过本回合出牌阶段`, 'skill');
    setTimeout(() => this.endPlayPhase(), 500);
    return;
  }
...
Game.prototype.aiRespondToSha = function(target, source, card) {
  ...
  // 赵云龙胆：杀可当闪
  const zhaoyunSha = target.hero.id === 'zhaoyun' ? target.hand.find(c => c.type === 'sha') : null;
  const guanyuRed = target.hero.id === 'guanyu' ? target.hand.find(c => isRedSuit(c.suit) && c.type !== 'sha') : null;
  const useShan = shanCard || (zhaoyunSha && (target.hp <= 2 || Math.random() < 0.7)) || (guanyuRed && (target.hp <= 2 || Math.random() < 0.7));

  if (useShan) {
    const used = shanCard || zhaoyunSha || guanyuRed;
    const isSkill = (target.hero.id === 'zhaoyun' || target.hero.id === 'guanyu') && used !== shanCard;
    ...
    if (pd && pd.shanNeeded <= 0) {
      ...
      this.addLog(`${target.hero.name}${isSkill ? `发动${skillName}` : ''}打出【闪】，成功闪避${totalNeeded > 1 ? '（无双）' : ''}`, isSkill ? 'skill' : '');
    } else if (pd) {
...
Game.prototype.aiRespondToAOE = function(target, source, requiredType) {
  ...
  } else if (target.hero.id === 'zhaoyun') {
    // 赵云龙胆：杀/闪互换
    card = target.hand.find(c => c.type === requiredType);
    if (!card) card = target.hand.find(c => (requiredType === 'sha' ? c.type === 'shan' : c.type === 'sha'));
  } else {
    card = target.hand.find(c => c.type === requiredType);
  }
  if (card && (target.hp <= 2 || Math.random() < 0.7)) {
    const isSkill = (target.hero.id === 'guanyu' || target.hero.id === 'zhaoyun') && card.type !== requiredType;
    const skillName = target.hero.id === 'zhaoyun' ? '【龙胆】' : '【武圣】';
    this.discardCard(target, card);
    this.addLog(`${target.hero.name}${isSkill ? `发动${skillName}` : ''}打出【${card.name}】`, isSkill ? 'skill' : '');
  } else {
...
Game.prototype.aiRespondJuedou = function(defender, challenger, lübuInvolved) {
  ...
  for (let i = 0; i < needed; i++) {
    let sha = defender.hand.find(c => c.type === 'sha' && !shaCards.includes(c));
    if (!sha && defender.hero.id === 'guanyu') {
      sha = defender.hand.find(c => isRedSuit(c.suit) && c.type !== 'sha' && !shaCards.includes(c));
    } else if (!sha && defender.hero.id === 'zhaoyun') {
      // 赵云龙胆：闪可当杀
      sha = defender.hand.find(c => c.type === 'shan' && !shaCards.includes(c));
    }
    if (sha) shaCards.push(sha);
  }
  if (shaCards.length >= needed) {
    const lastCard = shaCards[shaCards.length - 1];
    for (const c of shaCards) {
      const isSkill = (defender.hero.id === 'guanyu' || defender.hero.id === 'zhaoyun') && c.type !== 'sha';
