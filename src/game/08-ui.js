...
// 手牌数获取等供前端界面调用的通用工具函数。
  ...
  Game.prototype.calcDistance = function(from, to, forSha = false) {
    ...
    if (from.hero.id === 'machao') dist -= 1; // 马术：锁定技，与其他角色距离-1
  ...
  Game.prototype.dealDamage = function(target, source, amount, card = null) {
    ...
    for (let i = 0; i < amount; i++) {
      ...
      // 夏侯惇刚烈
      if (target.alive && target.hero.id === 'xiahoudun' && source && source !== target && source.alive && !this._ganglianGuard) {
        this.triggerGanglian(target, source);
      }

      // 司马懿反馈
      if (target.alive && target.hero.id === 'simayi' && source && source !== target && source.alive && !this._fankuiGuard) {
        const prev = this._fankuiGuard;
        this._fankuiGuard = true;
        this.triggerFankui(target, source);
        this._fankuiGuard = prev;
      }
  ...
  // 统一取消当前等待操作：清除等待状态并恢复出牌方回合（防止 AI 回合挂起）
  Game.prototype.cancelWait = function() {
    const wt = this.waitingForTarget;
    this.waitingForTarget = null;
    if (this.phase === 'play') {
      // 仍在出牌阶段：若出牌方是 AI/托管人类，恢复其出牌循环
      const source = (wt && (wt.source || wt.player)) || this.players[this.currentPlayerIdx];
      if (source && source.alive && (!source.isHuman || (source.isHuman && this.autoPlay))) {
  ...
  Game.prototype._doRender = function() {
    ...
    if (humanPlayer && humanPlayer.alive) {
      ...
      for (let i = 0; i < humanPlayer.hand.length; i++) {
        ...
        if (isHumanTurn && this.phase === 'play' && !this.waitingForTarget && !this.autoPlay
            && ((humanPlayer.hero.id === 'guanyu' && card.type !== 'sha' && isRedSuit(card.suit))
                || (humanPlayer.hero.id === 'zhaoyun' && card.type === 'shan'))) {
        ...
        if (this.waitingForTarget) {
          ...
          if (wt.type === 'shan_response') {
            ...
            else if (humanPlayer.hero.id === 'zhaoyun') clickable = (card.type === 'shan' || card.type === 'sha');
            ...
            else if (humanPlayer.hero.id === 'zhaoyun') clickable = (card.type === 'sha' || card.type === 'shan');
            ...
            else if (humanPlayer.hero.id === 'zhaoyun') clickable = (card.type === wt.requiredType
              || (wt.requiredType === 'sha' ? card.type === 'shan' : card.type === 'sha'));
    ...
    if (this.waitingForTarget && !this.autoPlay) {
      ...
      // 诸葛亮观星
      if (wt.type === 'guanxing') {
        const toBottom = wt.toBottom || [];
        html += `<span style="color:#80e0c0;margin-right:10px;">【观星】点击选择要置于牌堆底的牌（其余放回牌堆顶）：</span>`;
        for (let i = 0; i < wt.cards.length; i++) {
          const c = wt.cards[i];
          const isBot = toBottom.includes(i);
          html += `<button class="btn skill-btn" onclick="game.humanGuanxingToggle(${i})" style="border:${isBot ? '2px solid #80e0c0' : '1px solid #8b6914'};${isBot ? 'background:rgba(128,224,192,0.15);' : ''}">${c.suit}${this.getNumberStr(c.number)}【${c.name}】${isBot ? '⬇' : ''}</button>`;
        }
        html += '<button class="btn skill-btn" onclick="game.humanGuanxingConfirm()">确认观星</button>';
      }
      // 司马懿鬼才
      if (wt.type === 'guicai') {
        html += `<span style="color:#a0a0ff;margin-right:10px;">【鬼才】判定牌为${wt.judgeCard.suit}${this.getNumberStr(wt.judgeCard.number)}【${wt.judgeCard.name}】，是否用手牌替换？</span>`;
        for (let i = 0; i < wt.sima.hand.length; i++) {
          const c = wt.sima.hand[i];
          html += `<button class="btn skill-btn" onclick="game.humanGuicai(${i})" style="border:1px dashed #6060a0;">${c.suit}${this.getNumberStr(c.number)}【${c.name}】</button>`;
        }
        html += '<button class="btn" onclick="game.humanGuicaiSkip()">不发动</button>';
      }
      // 夏侯惇刚烈：来源是人类
      if (wt.type === 'ganglian_source') {
        html += `<span style="color:#ffa060;margin-right:10px;">${wt.target.hero.name}对你发动【刚烈】！选择：</span>`;
        html += `<button class="btn" onclick="game.humanGanglianChoice('discard')">弃置2张手牌</button>`;
        html += `<button class="btn danger" onclick="game.humanGanglianChoice('hurt')">受到1点伤害</button>`;
      }
      // 司马懿反馈：来源是人类
      if (wt.type === 'fankui_source') {
        html += `<span style="color:#a0a0ff;margin-right:10px;">${wt.target.hero.name}发动【反馈】，选择一张牌交给其：</span>`;
        for (let i = 0; i < wt.source.hand.length; i++) {
          html += `<button class="btn" onclick="game.humanFankuiPickHand(${i})" style="border:1px dashed #6060a0;">手牌${i + 1}</button>`;
        }
        const slots = ['weapon', 'armor', 'plusHorse', 'minusHorse'];
        const slotNames = { weapon: '武器', armor: '防具', plusHorse: '+1马', minusHorse: '-1马' };
        for (const s of slots) {
          if (wt.source.equipment[s]) {
            html += `<button class="btn" onclick="game.humanFankuiPickEquip('${s}')" style="border:1px dashed #6060a0;">${slotNames[s]}【${wt.source.equipment[s].name}】</button>`;
          }
        }
      }
      // 过河拆桥/顺手牵羊/破军盲选（出牌者看不见牌面，仅按位置选；装备/判定区可直接点选）
      if ((wt.type === 'guohe_discard' || wt.type === 'shunshou_steal' || wt.type === 'poujun_discard') && wt.blindPick) {
        const verb = wt.type === 'shunshou_steal' ? '顺走' : '弃置';
        const themeColor = wt.type === 'shunshou_steal' ? '#60d0ff' : '#f0d060';
        html += `<span style="color:${themeColor};margin-right:10px;">选择要${verb}${wt.target.hero.name}的牌：</span>`;
        ...
        const equipChoices = wt.choices.filter(c => c.type === 'equip');
        const slotNames = { weapon: '武器', armor: '防具', plusHorse: '+1马', minusHorse: '-1马' };
        for (const ec of equipChoices) {
          const handler = wt.type === 'shunshou_steal' ? 'humanShunshouStealEquip'
            : (wt.type === 'poujun_discard' ? 'humanPoujunDiscardEquip' : 'humanGuoheDiscardEquip');
          html += `<button class="btn" onclick="game.${handler}('${ec.slot}')" style="border:1px dashed #a08040;">${slotNames[ec.slot] || ec.slot}装备</button>`;
        }
        const judgeChoices = wt.choices.filter(c => c.type === 'judge');
        for (const jc of judgeChoices) {
          const handler = wt.type === 'shunshou_steal' ? 'humanShunshouStealJudge'
            : (wt.type === 'poujun_discard' ? 'humanPoujunDiscardJudge' : 'humanGuoheDiscardJudge');
          html += `<button class="btn" onclick="game.${handler}(${jc.idx})" style="border:1px dashed #a08040;">判定区【${jc.card.name}】</button>`;
        }
    ...
    if (isHumanTurn && this.phase === 'play' && !this.waitingForTarget && !this.autoPlay) {
      ...
      const skillDefs = {
        ...
        caoren: [{ id: 'jushou', name: '据守', cond: !this.jushouUsedThisTurn }],
      };
    ...
    if (this.waitingForTarget) {
      ...
      if (wt.type === 'shan_response') {
        if (card.type === 'shan' || (humanPlayer.hero.id === 'guanyu' && isRedSuit(card.suit))
            || (humanPlayer.hero.id === 'zhaoyun' && card.type === 'sha')) {
      ...
      if (wt.type === 'aoe_response') {
        if (card.type === wt.requiredType || (humanPlayer.hero.id === 'guanyu' && isRedSuit(card.suit))
            || (humanPlayer.hero.id === 'zhaoyun' && (wt.requiredType === 'sha' ? card.type === 'shan' : card.type === 'sha'))) {
      ...
      if (wt.type === 'juedou_defend_second' && (card.type === 'sha' || (humanPlayer.hero.id === 'guanyu' && isRedSuit(card.suit)) || (humanPlayer.hero.id === 'zhaoyun' && card.type === 'shan'))) { this.humanRespondJuedouSecond(card); return; }
